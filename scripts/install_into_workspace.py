...
- Treat `ts: next --app-server` as one direct invocation of `python3 <shed>/scripts/app_server_dispatch.py --workspace . next --app-server --json`; never wrap it in `codex exec` or another Codex agent. The dispatcher must reuse normal `next` navigation, require a valid campaign execution capsule, preflight Codex state/authentication/network/model access before mutation, and call the existing bounded Terra/medium `camp-run` path. Do not make `next` a role, change its selection, persist the preference, force GUI-native or non-executable work through CAMP execution, bypass compatibility, or add API fallback.
