...
- Treat `ts: reconcile campaigns` as a dry-run request for `reconcile_campaign_queue.py`. Apply only explicitly approved deterministic projection repairs with the reported state token; never apply its proposed execution order or resolve stalled lifecycle decisions implicitly.
