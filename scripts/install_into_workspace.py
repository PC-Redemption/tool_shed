...
- `work2`: perform `work1`, deploy to the configured work environment, run focused browser and changed-behavior checks, register the committed candidate and its open owning outcome chain with `release_cohort.py` when Hybrid state is active, persist its database checkpoint, and stop. The candidate is immediately available for local dogfooding but remains awaiting release.
...
- When Hybrid state has an active release cohort, bare `work5`/`ship` defaults to that accumulated cohort. Freeze its exact content SHA, attach verified publication evidence to every registered owner, reconcile only those outcome chains through their originating Ideas, and finalize the cohort only after every registered owner is terminal and reconciled. Do not infer ownership from all open Ideas.
...
- Treat `ts: queue` and `ts: status` as requests to read the active owner capsule and validate lifecycle state. When Hybrid state is active, also surface `release_cohort.py status`, including the active base tag, candidate count, lifecycle, and next Work2/Work5 transition.
...
- Resolve eligible App Server execution in this order: one-command `--gui`, strict one-command `--app-server`, the protected user-local preference changed by `ts: app-server on|off` (`appserver` alias), then the committed GUI default. A fresh schema-v2 `on` is operator-runtime trust for supported local roles, including bounded CAMP; unknown or updated Codex versions do not require positive qualification or executable-hash certification. Only an exact evidence-backed `unqualified` registry record denies a version in normal mode. Keep discussion, brainstorming, qualification workflows, and unsupported work GUI-native. Store active preference state only under Codex home, never in a workspace or installed snapshot. Explicit preference changes refresh a recovery-only owner profile outside Codex home; it never acts as consent without `ts: app-server profile restore`. Route validator defaults through `TOOL_SHED_STATE_ROOT`, and use `ts: app-server report` for the content-free source-aware local opportunity funnel.
- Route `ts: dashboard status|connect|connect poll|disconnect|report now|scheduler plan|scheduler install|scheduler remove` through the workspace-local `dashboard_reporter.py`. Verify the exact project and obtain operation-specific `dashboard-connect` or `dashboard-report` bindings before mutation. Keep reporter credentials private and user-local, never print or inspect them, and preserve local Tool Shed state as authority. Hosted reporting accepts only the strict allowlisted aggregate contract, uses a persistent idempotent outbox and singleton worker, and never blocks the originating local write. Enrollment approval and hosted views require password plus TOTP; credential and MFA provisioning remain explicit operator actions.
...
- For eligible `plan`, `verify`, and `camp-run`, the selector's allowed App Server JSON result creates a 300-second single-use dispatch lease. Pass `dispatch.correlation_id` to the immediately following matching `codex_orchestration.py` command as `--dispatch-correlation`, using the same non-default `--events` path when supplied. Never continue the selected action in GUI, omit the correlation, or replay an expired or consumed lease.
- Treat current-schema pending, expired, duplicated, malformed, or contradictory eligible dispatch lifecycles as closure debt. Strict Doctor, campaign/document completion, Work5 lane verification, and release-cohort freeze/publication/finalization fail closed until debt is dispositioned. After process loss, inspect Git and the mutation journal; use `app_server_control.py resolve <correlation> --disposition pre-mutation --json` only before any attempt, or `--disposition mutation-uncertain` after an attempt. Recovery is idempotent and never replays work. Pre-contract event schemas remain bounded historical telemetry.
- For eligible `ts: next`, invoke `python3 <shed>/scripts/app_server_dispatch.py --workspace . next --json` directly without a preceding standalone selector; `ts: next --app-server` adds the strict `--app-server` option. Never wrap either form in `codex exec` or another Codex agent. The dispatcher must own selection, attempt, and terminal accounting under one correlation identity, reuse normal `next` navigation and an existing valid capsule. When a ready campaign has no capsule, it assembles a deterministic focused snapshot from the campaign, project instructions, Git state, relevant file inventory, and bounded source excerpts; gives only that isolated snapshot to the supported read-only planning role without tools; validates and persists the resulting strict capsule through the guarded campaign transaction; and continues to the existing bounded Terra/medium `camp-run` path in the same invocation. The actual role operation performs startup, ChatGPT authentication, live-model, and requested-sandbox checks; do not add a parallel preflight system. Unsafe or indeterminate preparation fails before mutation. Do not make `next` a role, change its selection, force GUI-native or non-executable work through CAMP execution, bypass compatibility, or add API fallback.
...
- When every Hybrid campaign run and operation is terminal but stale non-runnable assignments keep the campaign active, use `campaign_execution.py status` and its read-only exact `plan`. Apply only a manifest with explicit administrative terminal disposition, reason, actor, authorization, a fresh project binding, and complete assignment classifications. Refuse runnable or nonterminal work and never infer outcome success; use normal complete, blocked, deferred, or abandoned routes when those semantics apply.
