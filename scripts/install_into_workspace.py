...
- Treat `ts: prm idea <idea-id-or-path>` as PRM sourced from that brief. Keep it `ready-for-prm` until settled project-map direction captures it, then set `promoted` and name that map in `Produces:`. Promotion preserves provenance and does not independently expand the active authority envelope.
...
- Resolve eligible App Server execution in this order: one-command `--gui`, strict one-command `--app-server`, the protected user-local preference changed by `ts: app-server on|off` (`appserver` alias), then the committed GUI default. Keep discussion, brainstorming, qualification gates, and unsupported work GUI-native. Store preference state only under Codex home, never in a workspace or installed snapshot.
