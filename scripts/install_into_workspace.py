...
- Treat `ts: reconcile campaigns` as a dry-run request for `reconcile_campaign_queue.py`. Report whole-work campaign coverage and exclusions as well as queue drift. Apply only an exact approved manifest with the reported whole-work state token; never apply its proposed execution order or resolve ambiguous lifecycle decisions implicitly.
