...
CANONICAL_PATH = Path("q&a") / "ask.txt"
...
def render(payload: dict[str, object]) -> str:
    ...
    if status == "canonical":
        return f"Using canonical inbox q&a/ask.txt:\n\n{payload['content']}"
    if status == "fallback":
        return (
            "Warning: actionable inbox content was found at noncanonical "
            "work/q&a/ask.txt; the canonical inbox is q&a/ask.txt.\n\n"
            f"{payload['content']}"
        )
    if status == "conflict":
        return (
            "Conflict: both q&a/ask.txt and work/q&a/ask.txt contain actionable content. "
            "Choose which request to use; the inboxes were not merged or modified."
        )
    return (
        "The Tool Shed Q&A inbox is empty: neither q&a/ask.txt nor work/q&a/ask.txt "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--workspace",
        default=".",
        help="Workspace root containing q&a/ and work/. Defaults to the current directory.",
    )
