#!/usr/bin/env python3
"""Schema-4 persisted outcome-loop finding resources."""
...
def create_loop_finding_schema(connection: sqlite3.Connection, *, include_triggers: bool = True) -> None:
    connection.executescript(LOOP_FINDING_SCHEMA_SQL)
    if include_triggers:
        connection.executescript(loop_finding_trigger_sql())


def loop_finding_migration_digest() -> str:
    return hashlib.sha256(
        (LOOP_FINDING_SCHEMA_SQL + "\n" + loop_finding_trigger_sql()).encode()
    ).hexdigest()
