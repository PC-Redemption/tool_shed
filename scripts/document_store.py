...
CHECKPOINT_FORMAT = 2
DOCUMENT_HYBRID_SCHEMAS = {HYBRID_SCHEMA_VERSION, 3, 4, 5, 6}
VISIBLE_ID = re.compile(r"^(?P<namespace>[A-Z]{2,5})-(?P<number>[0-9]{4,})$")
EDIT_HEADER = "tool_shed_document: 1"
LIFECYCLES = ("active", "working", "blocked", "parked", "deferred", "completed", "abandoned", "superseded", "terminal")
...
def domain_tables(connection: sqlite3.Connection) -> tuple[str, ...]:
    ...
    return tuple(
        table
        for table in (*hybrid_state.DOMAIN_TABLES, *DOCUMENT_TABLES, *closure_tables, *finding_tables, *execution_tables)
        if table in present
    )
...
def portable_tables(connection: sqlite3.Connection) -> tuple[str, ...]:
    ...
    execution_tables: tuple[str, ...] = ()
    if "campaign_execution" in present:
        from campaign_execution_schema import CAMPAIGN_EXECUTION_PORTABLE_TABLES

        execution_tables = CAMPAIGN_EXECUTION_PORTABLE_TABLES
    return tuple(
        table
        for table in (*PORTABLE_TABLES, *DOCUMENT_PORTABLE_TABLES, *closure_tables, *finding_tables, *execution_tables)
        if table in present
    )
...
TERMINAL_BODY_STATUSES = {
    ...
    "archived": {"archived"},
    "terminal": {"complete", "completed", "terminal", "reconciled-terminal", "not-satisfied"},
}
...
def _semantic_document_findings(connection: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if cycle["lifecycle_state"] == "terminal" and reconciliation and reconciliation["state"] == "reconciled":
            ...
            expected = (
                "superseded" if disposition == "superseded"
                else "terminal" if disposition in {"administratively-reconciled", "not-satisfied"}
                else "completed"
            )
        ...
        for child in child_rows:
            ...
            if not child_cycle or not (
                ...
                and child_cycle["disposition"] in {
                    ...
                    "administratively-reconciled", "not-satisfied"
                }
                and propagated
            ):
...
def rebuild(workspace: Path, *, project_binding: str, checkpoint: Path, output: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if hybrid_schema == 6:
            ...
            create_campaign_execution_schema(connection, include_triggers=False)
        ...
        if hybrid_schema == 6:
            ...
            ordered_portable.extend(CAMPAIGN_EXECUTION_PORTABLE_TABLES)
        ...
        if hybrid_schema == 6:
            from campaign_execution_schema import trigger_sql as campaign_execution_trigger_sql

            connection.executescript(campaign_execution_trigger_sql())
