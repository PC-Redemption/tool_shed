...
| `ts:work1` through `ts:work5`, aliases, `ts:check`, `ts:ship`, `ts: 100k`, `ts: prm <outcome>`, `ts: order`, `ts: autonomy`, numeric `ts: approve`, `ts: doctor`, campaign execution, owner campaign queues, Program Roadmaps, `ts: overview`, `ts: build focus areas`, hosted dashboard reporting, explicit App Server controls, `ts: help`, `ts: commands`, or `ts:ask` | `references/campaign-routes.md` |
