...
class LifecycleRouteSmokeTests(unittest.TestCase):
    def test_structural_oracle_accepts_exact_idempotent_route(self) -> None:
        ...
        completed = {"run_tag": "route-1", "database_revision": 20, "domain_digest": "d", "documents": docs, "relationships": relations, "cycles": cycles, "readiness_results": readiness, "closure_elements": {name: {"effective_closed": True} for name in ("ci", "cm", "cp", "cc")}, "projection_mismatches": []}
        result = smoke.evaluate(
            {"database_revision": 1}, completed, dict(completed), provider="openai", model="gpt", effort="low", turns=5, duration_seconds=10, adapter_version="1", platform_name="linux-x86_64"
        )
        self.assertEqual(result["verdict"], "PASS", result)

    def test_structural_oracle_rejects_duplicate_and_replay_write(self) -> None:
        completed = {"run_tag": "route-1", "database_revision": 20, "domain_digest": "d", "documents": [], "relationships": [], "cycles": [], "readiness_results": [], "closure_elements": {}, "projection_mismatches": []}
        replayed = {**completed, "database_revision": 21, "domain_digest": "changed"}
        result = smoke.evaluate(
            {"database_revision": 1}, completed, replayed, provider="openai", model="gpt", effort="low", turns=5, duration_seconds=10, adapter_version="1", platform_name="windows-amd64"
        )
