...
class SubprocessLaunchTests(unittest.TestCase):
    ...
    def test_windows_dashboard_qualification_covers_real_acceptance_signals(self) -> None:
        ...
        for expected in (
            "SetWinEventHook",
            'SetPhase("scheduled-safety")',
            'SetPhase("managed-write-burst")',
            "QueryFullProcessImageName",
            "Get-CimInstance Win32_Process",
            "$scriptsPath = $PSScriptRoot",
            "$identityPath --workspace $workspacePath identity",
            "$task.Actions[0].Execute",
            "scheduled dashboard task Python executable",
            "0x00041301",
            "scheduled safety pass was still running",
            "for _ in range(10)",
            "New-ScheduledTaskAction -Execute $pythonw",
            "$burstTaskName = \"ToolShedDashboardBurst-$burstId\"",
            "Unregister-ScheduledTask -TaskName $burstTaskName",
