...
class DocumentationSiteTests(unittest.TestCase):
    ...
    def test_workflow_help_teaches_nested_cycles_and_context_pages_link_to_it(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("ts: bs review IDEA-0008", ideas_help)
            self.assertIn("Exactly two triggers", ideas_help)
            self.assertIn("Ordinary brainstorming stays responsive", ideas_help)
            self.assertIn("transfer exactly", ideas_help)
            self.assertIn("One living Idea Brief", ideas_help)
            self.assertIn("immutable visible ID in SQLite", ideas_help)
            self.assertIn("Promotion can open an owning outcome loop", ideas_help)
