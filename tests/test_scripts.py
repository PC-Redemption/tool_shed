...
""",
    ...
    def test_all_python_cli_entrypoints_suppress_snapshot_bytecode(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            def ignore(_directory: str, names: list[str]) -> set[str]:
                ignored = {name for name in names if name in {".git", ".tool-shed", "work", "__pycache__"}}
    ...
    def test_operator_help_is_packaged_and_routed(self) -> None:
        ...
        for content in (guide, commands, skill_bundle):
            self.assertIn("ts: bs review", content)
            self.assertIn("CURRENT-READY", content)
            self.assertIn("ordinary", content.lower())
            self.assertIn("semantic", content.lower())
            self.assertIn("transfer", content.lower())
        installer = (ROOT / "scripts" / "install_into_workspace.py").read_text(encoding="utf-8")
        self.assertIn("exactly two triggers", installer)
        self.assertIn("ts: bs review", installer)
        self.assertIn("idea_readiness.py transfer-check", installer)
        self.assertIn("never perform semantic review", installer)
...
""",
    ...
    def test_executive_directive_preserves_ceo_text_as_ready_prm_input(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("- [x] Directive captured exactly", text)
            self.assertIn("## Recommended Next Action", text)
            self.assertIn("## Operator Needs", text)
            self.assertIn("## Completion Condition", text)
            payload = json.loads((workspace / "work/index.json").read_text(encoding="utf-8"))
