...
- Use `python3 scripts/reconcile_campaign_queue.py --workspace . --json` to inspect queue drift, whole-work campaign coverage, stalled campaigns, and proposed execution order without writing; applying changes requires an exact approved manifest and the reported whole-work token.
