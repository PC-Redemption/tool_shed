...
- `/guide/` is the ordered operating surface. Its new-project path can begin with the optional
  Brainstorm / Idea Brief Discovery Cycle before joining the shared Project Map → Program Roadmap
  → campaign generation → queue → execution → evidence loop.
- `/help/` and its direct-loadable topic paths explain individual Tool Shed concepts in depth.
  ...
  Plan → Roadmap → Milestone (PRM) outer lifecycle plus nested Program, Milestone Wave, Queue,
  Campaign, and Evidence cycles; contextual pages link back to it instead of making Campaigns own
  the whole model.
